...
//		$sql = "SELECT *  FROM ".$this->table." p join stock s on p.product_id=s.product_id";
//		return $this->db->query($sql)->result();
//	}
/*	public function read_all_products(){
		$sql ="SELECT p.product_id,p.product_image,p.product_name,p.sell_price ,s.stocks_qnty,st.supplied_stock,o.quantity
		FROM ".$this->table." p
		join stock s on p.product_id=s.product_id
		join (select * from supplier_transaction group by product_id ) st on st.product_id = p.product_id
		join (select b.payment_status,ord.product_id, ord.quantity  from billing b join order_details ord on b.order_id = ord.order_id  where b.payment_status = 'Paid' group by ord.product_id ) o on o.product_id = p.product_id";
	return $this->db->query($sql)->result();
	}*/
	public function read_all_products(){
		$sql ="SELECT p.product_id,p.product_image,p.product_name,p.sell_price ,s.stocks_qnty,st.supplied_stock,o.quantity
		FROM ".$this->table." p
		join stock s on p.product_id=s.product_id
		join (select * from supplier_transaction st group by product_id ) st on st.product_id = p.product_id
		join (select b.payment_status,ord.product_id, ord.quantity  from billing b join order_details ord on b.order_id = ord.order_id  where b.payment_status = 'Paid' group by ord.product_id ) o on o.product_id = p.product_id";
	return $this->db->query($sql)->result();
	}	
    ...
    public function view_supplier_transaction($product_id){
    	$this->db->select('*');
